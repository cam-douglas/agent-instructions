...
- Activate `/launch-pipeline` and `/instructions/LAUNCH.md` only when explicitly invoked or requested. Natural-language planning uses the matching instruction without implicitly starting the complete lifecycle.
- Run read-only preflight when execution is available. Run `bash .cursor/scripts/bootstrap.sh` only for required materialization or explicit installation/repair after implementation authorization; a READY workspace needs no repeat bootstrap.
- Use the adaptive gated role pipeline for an explicitly requested formal pipeline or consequential coordinated delivery. Within that pipeline, require role plans and evidence-backed handoffs under `docs/workstreams/`; bounded work needs no exhaustive role artifacts.
