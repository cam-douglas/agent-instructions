...
// Payloads are distributable copies, never an independent source of policy.
for (const [payload, canonical] of [
  ["bootstrap.sh", ".cursor/scripts/bootstrap.sh"],
  ["validate-agent-config.mjs", ".cursor/scripts/validate-agent-config.mjs"],
  ["policy.mjs", ".cursor/hooks/policy.mjs"],
  ["policy.test.mjs", ".cursor/hooks/policy.test.mjs"],
  ["cli.json", ".cursor/cli.json"],
  ["git-privacy-and-secrets.mdc", ".cursor/rules/git-privacy-and-secrets.mdc"],
]) {
  const payloadPath = `.cursor/skills/git-safety/payloads/${payload}`;
  try {
    expect(read(payloadPath) === read(canonical), `${payloadPath} has drifted from ${canonical}; regenerate the payload from its canonical source`);
  } catch {
    errors.push(`${payloadPath} or its canonical source is missing`);
  }
}

// Optional cross-client compatibility mirrors must not override the maintained
// Cursor skill with stale instructions. Consumers that copy only .cursor work.
for (const skill of ["supabase-linked-migrations", "vercel-deploy-workflow"]) {
  const mirror = `.agents/skills/${skill}`;
  try { statSync(pathFor(mirror)); } catch { continue; }
  function checkMirror(relative = "") {
    const canonicalDirectory = `.cursor/skills/${skill}${relative ? `/${relative}` : ""}`;
    for (const entry of readdirSync(pathFor(canonicalDirectory), { withFileTypes: true })) {
      if (entry.name === ".DS_Store") continue;
      const child = relative ? `${relative}/${entry.name}` : entry.name;
      if (entry.isDirectory()) checkMirror(child);
      else if (entry.isFile()) {
        try {
          expect(read(`${mirror}/${child}`) === read(`.cursor/skills/${skill}/${child}`), `${mirror}/${child} has drifted from the canonical Cursor skill`);
        } catch { errors.push(`${mirror}/${child} compatibility mirror is incomplete`); }
      }
    }
  }
  checkMirror();
